...
echo -e "${COLOR}Remove packages"
tput sgr0
cd /tmp
sudo apt -y purge firefox* kwrite vim
sudo apt -y autoremove

echo -e "${COLOR}Upgrade system"
tput sgr0
sudo apt -y update
sudo apt -y dist-upgrade

echo -e "${COLOR}Download and install Google Chrome"
tput sgr0
wget -c https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb
sudo dpkg -i google-chrome-stable_current_amd64.deb
sudo apt -y install -f

echo -e "${COLOR}Download and install Telegram"
tput sgr0
...
echo -e "${COLOR}Download and install QMPlay2"
tput sgr0
...
echo -e "${COLOR}Apply new icon theme"
tput sgr0
sed -i s/Theme=breeze/Theme=Papirus-Adapt -ya-Nokto/g ~/.config/kdeglobals

echo -e "${COLOR}Clear system cache"
tput sgr0
rm -rf ~/.cache/plasm* ~/.cache/ico*
sudo apt -y clean

echo -e "${COLOR}Restart Plasma Desktop"
tput sgr0
kquitapp5 plasmashell; kstart5 plasmashell; sleep 5

echo -e "${COLOR}Fix hardcode tray icons"
tput sgr0
sudo -E hardcode-tray --size 22 --conversion-tool RSVGConvert

echo -e "${COLOR}Fix hardcode apps icons"
tput sgr0
wget https://raw.githubusercontent.com/Foggalong/hardcode-fixer/master/fix.sh
sudo bash fix.sh

echo -e "${COLOR}Fix StartupWMClass"
tput sgr0
